 ...
 */
public class DarkChestBlock
        extends AbstractChestBlock<DarkChestBlockEntity>
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player2, Hand hand, BlockHitResult hit) {
        DarkChestInventory darkChestInventory = DarkChestBlockEntity.getDarkChestInventory();
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (darkChestInventory == null || !(blockEntity instanceof DarkChestBlockEntity)) {
        ...
        DarkChestBlockEntity darkChestBlockEntity = (DarkChestBlockEntity)blockEntity;
        darkChestInventory.setActiveBlockEntity(darkChestBlockEntity);
        player2.openHandledScreen(new SimpleNamedScreenHandlerFactory((syncId, inventory, player) -> GenericContainerScreenHandler.createGeneric9x3(syncId, inventory, darkChestInventory), CONTAINER_NAME));
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new DarkChestBlockEntity(pos, state);
    }

    @Override
    @Nullable
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? DarkChestBlock.checkType(type, ModBlockEntityType.DARK_CHEST, DarkChestBlockEntity::clientTick) : null;
    }
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof DarkChestBlockEntity) {
            ((DarkChestBlockEntity)blockEntity).onScheduledTick();
        }
    }
}
